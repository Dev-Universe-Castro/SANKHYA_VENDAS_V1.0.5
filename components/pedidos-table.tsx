...
export default function PedidosTable() {
  ...
  const loadPedidos = async () => {
    if (!searchNunota && !searchParceiro) {
      toast({
        title: "Atenção",
        description: "Informe o número do pedido ou nome do parceiro",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (searchNunota) params.append('nunota', searchNunota)
      if (searchParceiro) params.append('parceiro', searchParceiro)
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Falha ao carregar pedidos')
      }
      ...
      if (data.pedidos && data.pedidos.length > 0) {
        toast({
          title: "Sucesso",
          description: `${data.pedidos.length} pedido(s) encontrado(s)`,
        })
      } else {
        toast({
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-3 md:px-6 py-8 text-center text-sm text-muted-foreground">
                      Nenhum pedido encontrado. Use os filtros acima para buscar.
                    </td>
                  </tr>
                ) : (
                  pedidos.map((pedido) => (
